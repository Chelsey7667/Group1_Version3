//Group Members: Mariya Thomas, Chelsey Tharayil Moni, Taran Virk, Harshit Guglani and Ali Khorram
#define _CRT_SECURE_NO_WARNINGS
#define BUFFER_SIZE1 80
#define BUFFER_SIZE2 300
#define NUM_INPUT_SIZE 10
#include "cprversion3.h"
#include <stdio.h>
#include <string.h>
...
void converting(void) {
    ...
    char longString[BUFFER_SIZE1];
    long longNumber;
    do {
        printf("Type the long numeric string (q - to quit): \n");
        fgets(longString, BUFFER_SIZE1, stdin); //Reads a maximum of 80(BUFFER_SIZE) characters from the standard input and stores it in 'longString' character array
        longString[strlen(longString) - 1] = '\0'; //Assigns null terminator to the 'longString' array at the index number that precedes the index number which is equal to the length of the 'longString' character array 
        if (strcmp(longString, "q") != 0) { //Compares 'longString' with "q" lexicographically and executes the conditional loop if they are not the same
            longNumber = atol(longString); //Converts the string of numbers, "longString" to its equivalent long value and stores the value in 'longNumber'
            printf("Converted number is %d\n", longNumber);
        }
    } while (strcmp(longString, "q") != 0); //The 'do while' loop continues to execute until 'longString' matches with "q" lexicographically
...
void manipulating(void)
{
    ...
    // Declare variables
    char    haystack[BUFFER_SIZE1];
    char    needle[BUFFER_SIZE1];
    char* occurence = NULL;
    do {
        printf("Type the string (q - to quit):\n");
        fgets(haystack, BUFFER_SIZE1, stdin);
        haystack[strlen(haystack) - 1] = '\0'; // Remove newline character from input
        if (strcmp(haystack, "q") != 0) {
            // Call copyStrings function and copy source string to destination string
            printf("Type the substring:\n");
            fgets(needle, BUFFER_SIZE1, stdin);
            needle[strlen(needle) - 1] = '\0'; // Remove newline character from input
            // Use strstr function to find occurence of substring in main string
...
void fundamentals(void)
{
    ...
    // Declare Variables
    char destination[BUFFER_SIZE1];
    char source[BUFFER_SIZE1];
    do {
        destination[0] = '\0'; // reset destination string to empty
        printf("Destination string is reset to empty\n");
        printf("Type the source string (q - to quit):\n");
        fgets(source, BUFFER_SIZE1, stdin); // read string from user and store in buffer2
        source[strlen(source) - 1] = '\0'; // Remove newline character from input and replace with null terminator
        if (strcmp(source, "q") != 0) { // check if user inputed 'q'
            strcpy(destination, source);
            printf("New destination string is \'%s\'\n", destination);
        }
    } while (strcmp(source, "q") != 0); // Loop until user enters q
